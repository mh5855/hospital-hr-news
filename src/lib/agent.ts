...
const SEARCH_BATCHES = [
  [`종합병원 간호등급 변경 고시 ${YEAR}`, `보건의료노조 파업 쟁의 ${YEAR}`, `병원 포괄임금 폐지 근로기준법 ${YEAR}`],
  [`의료기관 노동법 판례 통상임금 ${YEAR}`, `PA간호사 제도화 법제화 ${YEAR}`, `당직의 전공의 근로시간 규제 ${YEAR}`],
  [`종합병원 채용 이직률 인력수급 ${YEAR}`, `4대보험료 변경 의료기관 ${YEAR}`, `대법원 의료기관 퇴직금 판결 ${YEAR}`],
  [`간호조무사 업무범위 변경 ${YEAR}`, `지방노동청 의료기관 근로감독 ${YEAR}`, `병원 최저임금 연차수당 ${YEAR}`],
]
    ...
    try {
      const response = await client.messages.create({
        ...
        messages: [
          {
            role: 'user',
            content: `다음 검색어로 2026년 기준 최근 변경사항 및 최신 종합병원 노무인사 뉴스를 검색하고 JSON 배열로 반환하세요. 2026년에 시행되었거나 예정된 변경사항을 우선적으로 수집하세요.
